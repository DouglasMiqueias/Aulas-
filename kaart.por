programa {
	...
	funcao devolverKart(inteiro qtdTotal, cadeia modelo[], inteiro disponivel[], real valor[], real &valorAcumulado[]) {
		inteiro numeroKart, achei = 0
		...
		para(inteiro i = 0; i < qtdTotal; i++) {
        		se(disponivel[i] == 0) { // Kart locado
        	 		escreva("Escolha ", i, " para devolver o kart: Modelo - ", modelo[i], "\n")
         			achei = 1
        	}
	}
	  		para(inteiro i = 0; i < qtdTotal; i++) {
			se(disponivel[i] != 0 e achei == 1) {
				escreva("Nenhum kart disponível para alugar\n")
				pare	
			}
	}
